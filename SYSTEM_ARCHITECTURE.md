...
CALC（月次・累計・前年同期・部門比較・科目内訳）
       ...
       +------> REPORT（部門別累計PLの参照表示）
       |
       +------> DASHBOARD（画面確認用の参照表示）
       |
       +------> EXECUTIVE_REPORT（社外・会議提出用の参照表示）
```
...
- EXECUTIVE_REPORT: 主要KPIと部門別前年差をA4横1枚へ要約する提出資料層

REPORTでDATAを直接集計せず、DASHBOARDとEXECUTIVE_REPORTで再集計しない。表示層の式は参照・検索・表示整形に限定する。
...
| EXECUTIVE_REPORT | executive report / PDF endpoint |
